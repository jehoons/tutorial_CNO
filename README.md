...
파이썬 레퍼는 내부적으로 rpy2를 실행합니다. rpy2 를 내부적으로 실행하는 과정에서 레포지토리를 찾지 못한다는 이야기가 나오면, `lib/python2.7/site-packages/rtools/package.py` 코드의  `cran_repos`변수를 "https://cloud.r-project.org/" 값으로 설정해 줍니다. 
